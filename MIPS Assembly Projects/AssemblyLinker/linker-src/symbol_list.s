...
addr_for_symbol_loop: 
    ...
    lw $a0, 0($s0) # First string for streq
    move $a1, $s1 # Second string for streq
    jal streq
    beq $v0, $0, addr_found
    lw $s0, 8($s0)
    j addr_for_symbol_loop
addr_found:
    lw $v0, 4($s0) # Load the address to return.
    lw $s1, 8($sp)
    lw $s0, 4($sp)
    lw $ra, 0($sp)
    addiu $sp, $sp, 12
...
add_to_list:
	...
	move $t2, $a2 # Address of symbol
	move $t3, $a0 # Pointer to next node
	
        move $a0, $a1 # Move string to by copied into $a0
        jal copy_of_str
        move $t1, $v0 # Pointer to copy of name
        
        jal new_node   # New node pointer will be in $v0 and contain 12 bytes
        
        sw $t1, 0($v0) # Insert name
        sw $t2, 4($v0) # Insert address
        sw $t3, 8($v0) # Insert pointer to next node
