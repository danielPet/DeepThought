...
hex_to_str_loop:
	beq $t0, 8, hex_to_str_end
	addiu $t1, $a0, 0          # Load integer in its current state to $t1
	srl $t1, $t1, 28           # Isolate top 4 bits
	bgt $t1, 9, hex_letter
	
	addiu $t1, $t1, 48         # ASCII: 0(48) - 9(57)
	sb $t1, 0($a1)             # Store ASCII value to buffer
	
	addiu	$a1, $a1, 1        # Point to next byte
	addiu $t0, $t0, 1          # Increment counter
	sll $a0, $a0, 4	           # Shift integer over 4 bits (so next 4 are ready to be read)
	j hex_to_str_loop
	
hex_letter:
	addiu $t1, $t1, 87         # ASCII: a(97) - z(122)
	sb $t1, 0($a1)
	
	addiu	$a1, $a1, 1        # Point to next byte
	addiu $t0, $t0, 1          # Increment counter
	sll $a0, $a0, 4	           # Shift integer over 4 bits
	j hex_to_str_loop

hex_to_str_end:
	addiu $t1, $0, 10
	sb $t1, 0($a1)             # Newline character
	addiu $a1, $a1, 1
	addiu $t1, $0, 0
	sb $t1, 0($a1)		   # NULL character
