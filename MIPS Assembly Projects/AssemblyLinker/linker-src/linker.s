...
write_machine_code:
	...
	# change this part.
	addiu $sp, $sp, -32
	sw $s6, 28($sp) # ADDED REGISTER for instruction
	sw $s5, 24($sp) # ADDED REGISTER for byte offset
	sw $s0, 20($sp)
	sw $s1, 16($sp)
	sw $s2, 12($sp)
	sw $s3, 8($sp)
...
write_machine_code_next_inst:
	...
	move $a0, $v1 # parse_int arguments: $a0 = string containing a number, $a1 = base (will be either 10 or 16)
	li $a1, 16
	jal parse_int
	move $s6, $v0 # Instruction moved to $s6
	
	# 4. Check if the instruction needs relocation. If it does not, branch to
	# the label write_machine_code_to_file:
	move $a0, $s6 # inst_needs_relocation argument: $a0 = 32-bit MIPS instruction, Returns: 1 if needs reloc, 0 if not
	...
	# relocate_inst arguments: $a0 = an instruction that needs relocating, $a1 = the byte offset of the instruction in the current file
	#  $a2 = the symbol table, $a3 = the relocation table
...
write_machine_code_to_file:
	...
	move $a0, $s6 # hex_to_str arguments: $a0 = int to write, $a1 = character buffer to write into
	la $a1, hex_buffer
	jal hex_to_str
