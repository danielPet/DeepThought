...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        frame_env = procedure.env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, frame_env)

    elif isinstance(procedure, MuProcedure):
        frame_env = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, frame_env)
...
def do_define_form(vals, env):
    ...
    check_form(vals, 2)
    target = vals[0]
    expr = vals.second.first
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        env.define(target, scheme_eval(expr, env))
        if not isinstance(target, str):
            raise SchemeError("bad name type")
        return target
    elif isinstance(target, Pair):
        name = target.first
        new_vals = Pair(target.second, vals.second)
        env.define(name, do_lambda_form(new_vals, env))
...
def do_let_form(vals, env):
    ...
    for term in bindings:
        if not isinstance(term.first, str):
            raise SchemeError()
        if len(term.second) > 1:
            raise SchemeError()
        names = Pair(term.first, names)
        eval_val = scheme_eval(term.second.first, env)
        values = Pair(eval_val, values)
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    predicate = scheme_true(scheme_eval(vals.first, env))
    consequent = vals.second.first
    if len(vals) == 3:
        alternative = vals.second.second.first
    else:
        alternative = okay
    if predicate:
        return consequent
...
def check_formals(formals):
    ...
    for symbol in formals:
        if not scheme_symbolp(symbol):
            raise SchemeError("formals contains invalid symbol(s)")
    for symbol in formals:
        for i in range(len(formals)):
            if symbol == formals[i] and symbol is not formals[i]:
                raise SchemeError("formals contains repeated symbol(s)")
