...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            "*** YOUR CODE HERE ***"
            if self.ant != None and self.ant.container and not insect.container and self.ant.ant  == None:
                self.ant.contain_ant(insect)
            elif self.ant != None and insect.container and not self.ant.container and insect.ant == None:
    ...
    def remove_insect(self, insect):
        ...
        elif insect.is_ant and not insect.name == 'Queen':
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if place != None and place.bees and place.name != 'Hive' and self.min_range <= locus and self.max_range >= locus:
...
class WallAnt(Ant):
    """WallAnt that blocks a tunnel until it is destroyed."""
    name = 'Wall'
    implemented = True
    food_cost = 4
...
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that can be placed on water."""
    name = 'Scuba'
    watersafe = True
    implemented = True
    food_cost = 5
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        if self.digesting > 0:
            self.digesting -= 1
        elif self.digesting == 0 and self.place.bees != None:
...
class QueenAnt(ScubaThrower): 
    ...
    def action(self, colony):
        ...
        #Below we iterate forward, then backward to double the damage of ants in the Queen's tunnel#

        position = self.place
        while position != None:
            if position.ant != self and position.ant != None and position.ant not in self.ants_in_tunnel:
                position.ant.damage *= 2
                self.ants_in_tunnel = self.ants_in_tunnel + [position.ant]
                
                if position.ant.container:
                    contained = position.ant.ant
                    if contained != self and contained != None and contained not in self.ants_in_tunnel:
                        position.ant.ant.damage *= 2
                        self.ants_in_tunnel = self.ants_in_tunnel + [position.ant.ant]
