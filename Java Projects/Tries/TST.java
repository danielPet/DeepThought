...
/**
 *  
 *  This is a modified TST class from Algorithms, 4th Edition 
 *  by Robert Sedgewick and Kevin Wayne.
 *
 *  I altered several of the methods below to use this class in
 *  my Autocomplete class.
 *  
...
public class TST<Value> {
    ...
    private static class Node<Value> implements Comparable{
        ...
        public int compareTo(Object obj) {
            Node<Value> n1 = (Node<Value>) obj;
            if (this.max - n1.max < 0) {
                return -1;
            } else if (this.max - n1.max > 0) {
                return 1;
            }
            return 0;
        }
     ...
     */
    public MaxPQ<TermNode> keys() {
        MaxPQ<TermNode> pq = new MaxPQ<TermNode>();
        System.out.println("here2");
        collect(root, new StringBuilder(""), pq);
        return pq;
    }
     ...
     *     as an iterable
     */
    public MaxPQ<TermNode> keysWithPrefix(String prefix) {
        MaxPQ<TermNode> pq = new MaxPQ<TermNode>();
        Node<Value> x;
        x = get(root, prefix, 0);
        if (x == null) return pq;
        if (x.val != null) pq.insert(new TermNode(prefix, (Double) x.val));
        collect(x.mid, new StringBuilder(prefix), pq);
        return pq;
    }

    // all keys in subtrie rooted at x with given prefix
    private void collect(Node<Value> x, StringBuilder prefix, MaxPQ<TermNode> pq) {
        if (x == null) return;
        collect(x.left, prefix, pq);
        if (x.val != null) {
            TermNode tn = new TermNode(prefix.toString() + x.c, (Double) x.val);
            pq.insert(tn);
        }
