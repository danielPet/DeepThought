...
class SimplePageRank(object):
    ...
    def update_weights(nodes, num_nodes):
        ...
        def distribute_weights((node, (weight, targets))):
            ...
            if len(targets) == 0:
                ...
                for t in range(0, num_nodes):
                    if t == node:
                        tup = (node, (0.05 * weight, []))
                    else:
                        tup = (t, (weight_contribution, []))
                    return_list.append(tup)
            else:
                weight_contribution = (0.85 * weight)/len(targets)
                for t in targets:
                    if t == node:
                        tup = (node, (0.05 * weight + weight_contribution, targets))
                    else:
                        tup = (t, (weight_contribution, []))
                    return_list.append(tup)

                if not node in targets:
                    tup = (node, (0.05 * weight, targets))
                    return_list.append(tup)
        ...
        def collect_weights((node, values)):
            ...
            return (node, (weights+0.1, targets))
